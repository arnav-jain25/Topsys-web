      ...
      <section style={{ paddingBottom: "8rem" }}>
        <div className="wrap">
          <div className="grid grid-cols-2 gap-5 max-[767px]:grid-cols-1">
            {INDUSTRIES.map(({ slug, label, body, tags }) => (
              <Link
                key={slug}
                href={`/industries/${slug}`}
                className="group block bg-white border border-hairline rounded-card px-8 py-8 transition-all duration-base ease-standard hover:border-transparent hover:shadow-e2 hover:-translate-y-[3px]"
              >
