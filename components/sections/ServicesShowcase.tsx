...
const SERVICES = [
  { ord: "01", href: "/capabilities/ai-and-data", title: "AI & data", Icon: IconAI },
  { ord: "02", href: "/capabilities/applications-and-modernization", title: "Applications & modernization", Icon: IconApps },
  { ord: "03", href: "/capabilities/cloud-and-platform-engineering", title: "Cloud & platform", Icon: IconCloud },
  { ord: "04", href: "/capabilities/cybersecurity", title: "Cybersecurity", Icon: IconSecurity },
  { ord: "05", href: "/capabilities/technology-talent", title: "Technology talent", Icon: IconTalent },
];
...
export function ServicesShowcase() {
  ...
  return (
    <div ref={ref} className="mt-16">
      ...
      <div className="relative grid grid-cols-5 gap-3 max-[900px]:grid-cols-3 max-[600px]:grid-cols-2">
        {/* Threaded rail behind the icons */}
        <div
          className="absolute left-0 right-0 hidden min-[901px]:block"
          style={{ top: "34px", height: "1px", background: "var(--color-hairline)" }}
          aria-hidden="true"
        />
        {SERVICES.map(({ ord, href, title, Icon }, i) => (
          <Link
            key={href}
            href={href}
            className="group relative flex flex-col items-center text-center px-2 py-2"
            style={
              visible
          ...
          >
            <span
              className="relative z-[1] flex items-center justify-center w-[52px] h-[52px] rounded-card bg-paper border border-hairline text-ink-muted transition-all duration-base ease-standard group-hover:border-teal group-hover:text-teal group-hover:-translate-y-[3px] group-hover:shadow-e1"
            >
              <Icon />
            </span>
            ...
            <span className="mt-1.5 font-display font-medium text-body-sm text-ink group-hover:text-teal transition-colors duration-fast ease-standard">
              {title}
            </span>
          </Link>
        ))}
